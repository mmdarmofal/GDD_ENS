...
def process_data_folds_orig(n_splits, fold): 
	''' 
	...
	'''
	data_train = pd.read_csv('ft_train_orig.csv', sep = ',', index_col = 0)
	labels_train = pd.read_csv('labels_train_orig.csv', sep = ',', index_col = 0).squeeze('columns')
	...
	return fold_train_x, fold_train_y, fold_val_x, fold_val_y, n_features, n_types
...
if __name__ == "__main__":
	...
	x_train, y_train, x_val, y_val, n_features, n_types = process_data_folds_orig(n_splits, fold)
	...
	path_best_model = 'fold_' + str(fold+1) + '_orig.pt' #saves the best found model at this path
	path_best_params = 'fold_' + str(fold+1) + '_orig_params.csv' #saves the best found params
	best_accuracy = 0.0 
	n_it = 0
	...
	search_result = gp_minimize(func=fitness, dimensions=dimensions, acq_func='gp_hedge', n_calls=300, x0=default_paramaters, random_state=0, n_jobs = -1, callback = Iteration_Stop(max_upd = 1)) 
